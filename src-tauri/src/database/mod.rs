...
/// Generate definition_review.csv if it doesn't exist
/// This is called automatically on app startup to provide users with a review list
fn generate_definition_review_if_needed() -> std::result::Result<(), Box<dyn std::error::Error>> {
    let review_path = if cfg!(debug_assertions) {
    ...
    if review_path.exists() {
        println!("[DB] Definition review file already exists");
        return Ok(());
    }

    println!("[DB] Generating definition review CSV...");

    // Parse CEDICT to find duplicates
    let cedict_path = if cfg!(debug_assertions) {
        PathBuf::from("../datasets/cedict_ts.u8")
    } else {
        PathBuf::from("datasets/cedict_ts.u8")
    };
    ...
    let cedict_entries = match cedict::parse_cedict_file(cedict_path.to_str().unwrap()) {
        Ok(entries) => entries,
        Err(e) => {
            println!("[DB] Warning: Could not parse CEDICT: {}", e);
            return Ok(());
        }
    };
    ...
    // Find characters with multiple DISTINCT entries
    ...
    for (id, character, _simplified, pinyin, definition, is_word, freq_rank) in items {
        ...
        // Flag: Very long (but only with other flags)
    ...
    println!("[DB] Found {} items needing review", review_items.len());

    // Write to CSV
    use std::io::Write;
    let mut file = match fs::File::create(&review_path) {
        Ok(f) => f,
        Err(e) => {
            println!("[DB] Warning: Could not create review file: {}", e);
            return Ok(());
        }
    };
